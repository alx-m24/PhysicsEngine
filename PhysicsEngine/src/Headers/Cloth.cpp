...
{
	sf::Vector2f pos = { 0, 0 };
	particles->push_back(new Particle(pos, sf::Color::White, 10.0f, 2.0f, { 400, 100 }));
...
String::String(std::vector<Particle*>* particles, std::vector<Spring*>* springs, unsigned int count, float spacing, float strength)
{
	...
	for (int i = 1; i < count; ++i) {
		...
		springs->push_back(new Spring((*particles)[particles->size() - 1], (*particles)[particles->size() - 2], spacing, strength));
	}
}

Bridge::Bridge(std::vector<Particle*>* particles, std::vector<Constriant*>* constraints, unsigned int count, float spacing)
{
	sf::Vector2f pos = { 0, 0 };
	particles->push_back(new Particle(pos, sf::Color::White, 10.0f, 2.0f, { 400, 100 }));
	for (int i = 1; i < count; ++i) {
	...
	particles->push_back(new Particle(pos, sf::Color::White, 10.0f, 2.0f, { 800, 100 }));
	constraints->push_back(new Constriant((*particles)[particles->size() - 1], (*particles)[particles->size() - 2], spacing));
}
